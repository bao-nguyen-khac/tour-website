...
const RatingsReviews = () => {
    ...
    try {
      ...
      let url =
        filter === "most" //most rated
          ? `${apiUrl}/api/package/get-packages?searchTerm=${search}&sort=packageTotalRatings`
          : `${apiUrl}/api/package/get-packages?searchTerm=${search}&sort=packageRating`; //all
  ...
  const onShowMoreSClick = async () => {
    ...
    let url =
      filter === "most" //most rated
        ? `${apiUrl}/api/package/get-packages?searchTerm=${search}&sort=packageTotalRatings&startIndex=${startIndex}`
        : `${apiUrl}/api/package/get-packages?searchTerm=${search}&sort=packageRating&startIndex=${startIndex}`; //all
    const res = await fetch(url);
    const data = await res.json();
    ...
    <>
      <div className="shadow-xl rounded-lg w-full flex flex-col p-5 justify-center gap-2">
        ...
        {packages ? (
          ...
          <h1 className="text-center text-2xl">Chưa có đánh giá nào!</h1>
        )}
        {showMoreBtn && (
          <button
