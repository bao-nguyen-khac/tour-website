...
const RatingsPage = () => {
  ...
  const getRatings = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${apiUrl}/api/rating/get-ratings/${params.id}`, {
        headers: {
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
        },
      });
      const res2 = await fetch(`${apiUrl}/api/rating/average-rating/${params.id}`, {
        headers: {
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
        },
      });
    ...
    <div className="w-full p-3">
      <div className="w-full">
        ...
        {!loading && !packageRatings && (
          <h1 className="text-center text-2xl">Không tìm thấy đánh giá nào!</h1>
        )}
        {!loading && packageRatings && (
